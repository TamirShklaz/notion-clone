    ...
    return (
        ...
        >
            {!!id && (
                ...
                >
                    <ChevIcon className={"h-4 w-4 shrink-0 text-muted-foreground/50"}/>
                </div>
            )}
            {documentIcon ? (
            ...
            {isSearch && (
                <kbd
                    className={"ml-auto pointer-events-none inline-flex h-5 select-none items-center " +
                        "gap-1 rounded border bg-muted px-1.5 font-medium text-muted-foreground"}>
...
Item.Skeleton = function ItemSkeleton({level}: { level?: number }) {
    return (
        <div style={{paddingLeft: level ? `${(level * 12) + 25}px` : "12pxs"}} className={"flex gap-x-2 py-[3px]"}>
